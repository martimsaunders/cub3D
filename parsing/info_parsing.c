...
{
    if (*line == 'N')
		return (get_tex_path(++line, &pc()->image.wall_n.path,
				&ps()->n_tex));
	if (*line == 'S')
		return (get_tex_path(++line, &pc()->image.wall_s.path,
				&ps()->s_tex));
	if (*line == 'E')
		return (get_tex_path(++line, &pc()->image.wall_e.path,
				&ps()->e_tex));
	if (*line == 'W')
		return (get_tex_path(++line, &pc()->image.wall_o.path,
				&ps()->o_tex));
    return (2);
}
...
int	find_map_info(char *line)
{
	...
	if (line[i] == 'F')
		return (get_hex_num(&line[i + 1], &pc()->image.floor,
				&ps()->c_floor));
	if (line[i] == 'C')
		return (get_hex_num(&line[i + 1], &pc()->image.ceiling,
				&ps()->c_ceil));
	if (line[i] == '1')
		return (2);
