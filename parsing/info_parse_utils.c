...
bool	valid_color_nums(int i, char **nums, int rgb[3])
{
	...
	if (i == 3)
		return (err_msg("Invalid color information (C/F R,G,B)", NULL), false);
	j = 0;
	while (nums[i][j + 1])
	{
		if (j == 3)
			return (err_msg("Invalid color number (0 a 255)", NULL), false);
		if (!ft_isdigit(nums[i][j]))
			return (err_msg("Invalid color information (C/F R,G,B)", NULL),
				false);
		j++;
	}
